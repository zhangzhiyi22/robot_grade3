...
class ModbusRTUMaster(Node):
    ...
    def __init__(self):
        ...
        self.__modbus_publisher = self.create_publisher(UInt8MultiArray, 'serial_write', 20)
        self.__modbus_subscription = self.create_subscription(UInt8MultiArray, 'serial_read', self._modbus_callback, 20)

        self.__imu_publisher_ = self.create_publisher(Imu, 'external_imu', 10)
        # self.__magnetic_publisher = self.create_publisher(MagneticField, 'magnetic', 10)

        timer_period = 0.01
        self.timer = self.create_timer(timer_period, self._timer_callback)

    def _timer_callback(self):
        # self.get_logger().info('abc: %s' % self.__basic_updated)
        # self.get_logger().info('def: %s' % self.__orient_updated)
